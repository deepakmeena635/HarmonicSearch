...
__global__ void objectiveFn(float *res, float *harmonics, int dim,
...
__global__ void sorted(float *res, float *harmonics, float *obj, float *sortedObj, int population , int dim, int pitch){
...
__global__ void harmonic_update(float *harmonics, float *bests,float* noise,
...
__global__ void update_harmonics( float *harmonics,float *bests,
...
__global__ void cast_to_int(int *res, float *arr,int rows, int maxIndex){
	int id = blockIdx.x*blockDim.x+threadIdx.x;
	if(id>= rows){return ;}
...
__global__ void copy_vec( float *a, float *b, int astart, int bstart, int dim ){
